...
export function SettingsForm({ initial }: { initial: SiteSettingsView }) {
  ...
  async function save() {
    if (coverBusy) {
      toast.info("대표 사진을 올리는 중이에요. 잠시만 기다려 주세요.");
      return;
    }
    setSaving(true);
    ...
    toast.success("전시 정보를 저장했어요.");
    router.refresh();
  }

  return (
    <div className="mx-auto max-w-3xl px-4 pb-32 pt-7 sm:px-6 sm:pt-10">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="font-serif text-[1.9rem] font-bold tracking-[-0.02em] text-ink">전시 정보</h1>
          <p className="mt-1 text-[15px] text-ink-soft">전시 표지와 공유 미리보기에 쓰이는 정보예요.</p>
        </div>
      ...
      <div className="mt-7 space-y-6">
        <Card title="전시">
          <div className="space-y-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <TextField label="시작일" optional type="date" value={form.startDate} onChange={(e) => set("startDate", e.target.value)} error={errors.startDate} />
              <TextField label="종료일" optional type="date" value={form.endDate} onChange={(e) => set("endDate", e.target.value)} error={errors.endDate} />
            </div>
        ...
        <Card title="소개글">
          <TextArea label="전시 소개" optional value={form.intro} onChange={(e) => set("intro", e.target.value)} maxLength={3000} placeholder="전시를 준비한 마음, 관람객에게 전하고 싶은 말을 적어 주세요." error={errors.intro} />
        </Card>

        <Card title="대표 사진" hint="전시 표지와 카카오톡 공유 미리보기에 쓰여요.">
