...
test("틀린 비밀번호는 한국어로 안내하고, 맞으면 들어간다", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "들어가기" }).click();
  await expect(page.getByRole("alert").filter({ hasText: "비밀번호가 맞지 않아요" })).toBeVisible();
...
test("사진을 올려 작품을 등록하면 WebP로 저장되고 QR 주소가 생긴다", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "등록하기" }).first().click();
  await expect(page.getByText("작품을 등록했어요.", { exact: false })).toBeVisible();
...
test("비공개로 바꾸면 QR 페이지는 '준비 중' 화면을 보여준다", async ({ page }) => {
  ...
  await expect(page.getByText("비공개로 바꿨어요", { exact: false })).toBeVisible();

  await page.goto(artworkUrl);
  await expect(page.getByRole("heading", { name: "작품 이야기를 준비하고 있어요" })).toBeVisible();

  await page.goto("/admin");
  await page.getByRole("switch", { name: "준비 중" }).first().click();
  await expect(page.getByText("관람객에게 공개했어요.")).toBeVisible();
});
...
test("없는 작품 주소는 한국어 안내 화면을 보여준다", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "지금은 볼 수 없는 작품이에요" })).toBeVisible();
  await expectNoEnglishErrors(page);
});
...
test("삭제는 디자인된 확인 모달을 거친다", async ({ page }) => {
  ...
  await expect(dialog).toContainText("삭제할까요?");
